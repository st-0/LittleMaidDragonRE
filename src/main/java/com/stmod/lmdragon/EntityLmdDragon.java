...
public class EntityLmdDragon extends EntityTameableDragon {
	...
	public static boolean switchDragon(EntityPlayer player, EntityLmdMaidsan maid) {
		...
		dragon.setLocationAndAngles(maid.posX, maid.posY, maid.posZ, MathHelper.wrapDegrees(player.worldObj.rand.nextFloat() * 360.0F), 0);
	...
	public static boolean switchToMaidsan(EntityPlayer player, EntityLmdDragon dragon) {
		...
		maid.setLocationAndAngles(dragon.posX, dragon.posY, dragon.posZ, MathHelper.wrapDegrees(player.worldObj.rand.nextFloat() * 360.0F), 0);
